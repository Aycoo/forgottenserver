...
bool Spell::playerRuneSpellCheck(Player* player, const Position& toPos)
{
	...
	if (toPos.x != 0xFFFF) {
		...
		if (playerPos.z > toPos.z) {
			...
			if (isAggressive && needTarget && (player->getConfigBoolean(PLAYER_NO_SECURE_MODE) == false && player->getSecureMode() == SECUREMODE_ON) && topVisibleCreature) {
...
bool RuneSpell::executeUse(Player* player, Item* item, const PositionEx& posFrom, const PositionEx& posTo, bool, uint32_t creatureId)
{
	...
	if (result) {
		...
		if (hasCharges && item && player->getConfigBoolean(PLAYER_REMOVE_RUNE_CHARGES)) {
